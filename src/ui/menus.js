...
export class UI {
  ...
  showTitle() {
    ...
    this.show(`
      <div class="title-wrap">
        <div class="title-mark">呪</div>
        <h1 class="title">NEXUS ISLAND ROYALE</h1>
        <h2 class="subtitle">CURSED — 呪術戦</h2>
        ...
        <div class="menu-list">
          <button class="menu-btn primary" data-act="play" data-sound>
            <span class="jp">出撃</span><span class="en">Start a Fight</span>
          </button>
          <button class="menu-btn" data-act="codex" data-sound>
            <span class="jp">呪術書</span><span class="en">Codex — techniques &amp; mechanics</span>
          </button>
          <button class="menu-btn" data-act="settings" data-sound>
            <span class="jp">設定</span><span class="en">Settings</span>
          </button>
        </div>
    ...
    const render = () => {
      ...
      const abilities = t.abilities.map((a, i) => `
        <div class="ab">
          ...
          <div class="ab-body">
            <div class="ab-name">${a.name} <span class="ab-jp">${a.jp || ''}</span>
      ...
      const domain = t.domain ? `
        <div class="domain-block" style="border-color:${t.domain.color}55">
          <div class="dom-jp" style="color:${t.domain.color}">${t.domain.jp}</div>
      ...
      this.$('#charDetail').innerHTML = `
        <div class="cd-head">
          ...
          <div class="cd-tech" style="color:${t.color}">${t.name}<span>${t.jp}</span></div>
        </div>
        ...
        <div class="passive">
          <div class="p-title">Passive — ${t.passive.name} <span>${t.passive.jp || ''}</span></div>
  ...
  showLoadout() {
    ...
    const tools = TOOL_LIST.map((id) => {
      ...
      return `<button class="card tool-card ${this.selection.tool === id ? 'sel' : ''}" data-tool="${id}" data-sound>
        <div class="card-jp" style="color:${tool.color}">${tool.jp}</div>
    ...
    const vows = VOW_LIST.map((v) => `
      <button class="card vow-card ${this.selection.vows.includes(v.id) ? 'sel' : ''}" data-vow="${v.id}" data-sound>
        <div class="vow-head"><span class="card-jp">${v.jp}</span><span class="vow-cost">${v.cost}</span></div>
    ...
    this.show(`
      <div class="panel wide-panel">
        ...
        <button class="menu-btn primary wide" data-act="fight"><span class="jp">領域展開</span><span class="en">Begin</span></button>
      </div>
    `);
  ...
  showPause() {
    ...
    this.show(`
      <div class="panel narrow">
        ...
        <div class="menu-list">
          <button class="menu-btn primary" data-act="resume"><span class="jp">続行</span><span class="en">Resume</span></button>
          <button class="menu-btn" data-act="codex"><span class="jp">呪術書</span><span class="en">Codex</span></button>
          <button class="menu-btn" data-act="settings"><span class="jp">設定</span><span class="en">Settings</span></button>
          <button class="menu-btn danger" data-act="quit"><span class="jp">撤退</span><span class="en">Abandon mission</span></button>
        </div>
      </div>
    `, 'overlay');
  ...
  showImpromptuVows(fighter, onPick) {
    ...
    this.show(`
      <div class="panel">
        <h2 class="vow-title">縛り — Declare a Binding Vow</h2>
...
const CODEX_RENDER = {
  mechanics: () => `
    <article>
      ...
      <h3>Reverse Cursed Technique — 反転術式</h3>
  ...
  techniques: () => Object.values(TECHNIQUES).map((t) => `
    <article class="codex-tech">
      <h3 style="color:${t.color}">${t.name} <span class="jp">${t.jp}</span></h3>
      <div class="fam">${t.family}</div>
      <p>${t.blurb}</p>
      <div class="passive"><b>${t.passive.name}</b> — ${t.passive.desc}</div>
      <ul class="abl">
        ${t.abilities.map((a) => `<li><b>${a.name}</b> <span class="jp">${a.jp || ''}</span> — ${a.desc}
          <span class="dim">(${a.cost} CE, ${a.cooldown}s)</span></li>`).join('')}
      </ul>
      ${t.domain
        ? `<div class="dom"><b>領域展開 ${t.domain.name}</b> — ${t.domain.desc}<br><i>${t.domain.blurb}</i></div>`
        : `<div class="dom none"><b>No domain</b> — ${t.blurbNoDomain}</div>`}
    </article>`).join(''),

  domains: () => `
    <article>
      <h3>Domain Expansion — 領域展開</h3>
      ...
      <ol>
        <li><b>Simple Domain (E)</b> — 簡易領域. A 2.21m circle. Neutralises the sure-hit while you stand in it and
        auto-counters anything crossing the line. Drains energy fast and slows you to a walk.</li>
        <li><b>Domain Amplification (Z)</b> — 領域展延. Barrier wrapped over your body. Technique effects fizzle on
        contact for 2.8s, and your own attacks pierce Infinity.</li>
        <li><b>Falling Blossom Emotion</b> — 落花の情. Mastered Simple Domain users repel the enemy barrier on
  ...
  vows: () => `
    <article>
      ...
      ${VOW_LIST.map((v) => `<div class="vow-row">
        <b>${v.name}</b> <span class="jp">${v.jp}</span> <span class="dim">(${v.cost} pts)</span>
      ...
      ${IMPROMPTU_VOWS.map((v) => `<div class="vow-row">
        <b>${v.name}</b> <span class="jp">${v.jp}</span>
  ...
  tools: () => `<article>${TOOL_LIST.map((id) => {
    ...
    return `<div class="tool-row">
      <b style="color:${t.color}">${t.name}</b> <span class="jp">${t.jp}</span>
      <div>${t.desc}</div>
  ...
  bestiary: () => `<article>${Object.values(CURSES).map((c) => `
    <div class="curse-row">
      <b style="color:${c.color}">${c.name}</b> <span class="jp">${c.jp || ''}</span>
